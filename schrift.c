...
static void *reallocarrays(void *optr, size_t nmemb, size_t size);
...
grow_points(Outline *outl)
{
	...
	if (!(mem = reallocarrays(outl->points, cap, sizeof *outl->points)))
		return -1;
...
grow_curves(Outline *outl)
{
	...
	if (!(mem = reallocarrays(outl->curves, cap, sizeof *outl->curves)))
		return -1;
...
grow_lines(Outline *outl)
{
	...
	if (!(mem = reallocarrays(outl->lines, cap, sizeof *outl->lines)))
		return -1;
